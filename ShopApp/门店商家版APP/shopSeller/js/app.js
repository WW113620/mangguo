...
(function($, app) {
	...
	app.timeCalculte=function(d,el){
		var nowDate=app.timeFormat('','arg'),
			year=nowDate.year,
			month=nowDate.month,
			day=nowDate.day,
		...
		switch(d){
			case '0':
				//resultStart=resultEnd=year+'-'+month+'-'+day;
				resultStart=resultEnd=''; //今日实时不传日期
				break;
			case '1':
				d = parseInt(d);
		        var f = new Date(),
		        	f = f.setDate(f.getDate()-d);
		        resultStart=resultEnd=app.timeFormat(f).substring(0,10);
				break;
			default:
				d = parseInt(d);
		        var f = new Date();
		        resultEnd = app.timeFormat(f.setDate(f.getDate()-1)).substring(0,10);
	        	f = f.setDate(f.getDate()-d+1);
		        resultStart=app.timeFormat(f).substring(0,10);
				break;
