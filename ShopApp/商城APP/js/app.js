...
//var URL = 'https://demo.himall.kuaidiantong.cn/',
var URL = 'http://himall33.himall.kuaidiantong.cn/',
...
(function($, app) {
	...
	app.templateHref = function() {
		mui('#modules').on('tap', 'a[href]', function() {
			...
			if(href.indexOf('product/detail/') >= 0) {
				...
				id = href.split('?productid=')[1];
				showProduct(id);
			} else if(href.indexOf('topic/detail/') >= 0) {
				id = href.split('topic/detail/')[1];
				var topicDetail = plus.webview.getWebviewById('topic-detail.html');
				if(!topicDetail) {
					himall.openVW('topic-detail.html', {
						topicId: id
					});
				} else {
					mui.fire(topicDetail, 'updateData', {
						topicId: id
					});
				}
			} else if(href.indexOf('gift/detail/') >= 0) {
				...
				himall.openVW('merge-detail.html', {
					activeId: id
				});
			...
			} else if(href.indexOf('shopregister/step1') >= 0) {
				himall.openVW('web-show.html', {
					link: URL + 'm-wap/ShopRegisterJump/AppJump?tourl=1',
					name: '商家入驻'
				});
		...
		mui('#modules').on('submit', 'form', function() {
			return false;
		});
	...
	//上传图片处理
	app.uploadImage = function(content, callback) {
		var wt,
			imgBox,
			upload = function(picStr) {
			...
			appendByGallery = function() { //从相册添加文件
				plus.gallery.pick(function(path) {
					appendFile(path); //处理图片的地方
				});
			},
			getImage = function() { //拍照添加文件
		...
		mui(content).on('tap', '.upload-item i', function() {
			document.activeElement.blur();
		...
		mui(content).on('tap', '.upload-item .del', function() {
...
function initPay(dcontent) {
	plus.payment.getChannels(function(channels) {
		...
		var haswxPay = false;
		var hasaliPay = false;
		mui.ajax(URL + 'api/payment/GetEnablePaymentList', {
			data: himall.md5Data({
				userkey: himall.getState().userkey
			}),
			dataType: 'json',
			...
			success: function(data) {
				...
				if(data.data.length > 0) {
					for(var i = 0; i < data.data.length; i++) {
						...
						console.log(item.id);
						if(item.id.indexOf("WeiXinPay_App") > -1) {
							haswxPay = true;
						}
						if(item.id.indexOf("Alipay_App") > -1) {
							hasaliPay = true;
						}
					}
				}
				content.innerHTML = (haswxPay ? wxpayStr : "") + (hasaliPay ? alipayStr : "");
			}
		});
	}, function(e) {
...
function shareMessage(share, ex, params) {
	...
	if(ex == "WXSceneSession" || ex == "WXSceneTimeline") {
		msg.type = "web";
	}
...
var commonFn = {
	favShop: function() {
		mui('.mui-content').on('tap', '.favShop', function(e) {
			e.stopPropagation();
			var _this = this;
			if(himall.isLogin()) {
				var w = plus.nativeUI.showWaiting('', {
				...
				mui.ajax(URL + 'api/VShop/PostAddFavoriteShop', {
					...
					success: function(data) {
						w.close();
						if(data.success) {
							if(himall.hasClass(_this, 'icon-aixin')) {
								_this.className = _this.className.replace('aixin', 'guanzhu');
							} else {
								_this.className = _this.className.replace('guanzhu', 'aixin');
							}
						}
...
function customerService(data) {
	...
	div.innerHTML = '<div class="keng-lb-1"><i class="iconfont icon-service"></i></div><div style="width:' + ((data.length * 36) + 54) + 'px;display:none;" class="service_list">' + html + '</div>';
...
var toMain = function() {
	...
	if(isEmptyObject(params)) {
		...
		if(params.fireId && !params.id) {
			mui.fire(plus.webview.getWebviewById(params.fireId), 'updateData');
			setTimeout(function() {
				ws.close();
			}, 300)
		} else {
